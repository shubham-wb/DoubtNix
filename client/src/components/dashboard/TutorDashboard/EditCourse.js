...
function EditCourse() {
  let { id } = useParams();
  console.log(id);
  let course = useCourses();

  let [lessons, setLessons] = useState([]);

  useEffect(() => {
    const getLesson = async () => {
      const response = await course.getCourseById(id);
      setLessons((lessons = response.data.lessons));
    };
    getLesson();
  }, []);

  const handlePublish = async () => {
    let response = await course.publishMyCourse(id);
    if (response.success) {
  ...
  const handleDelete = async () => {
    let response = await course.deleteOne(id);
    if (response.success) {
      alert("course published");
    }
